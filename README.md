Updated on April 9
# Hi, I'm Sophia Castro! 👩🏻


## 🚀 About Me
QA Engineer in training, focused on manual testing, API testing, and test automation with Python. I have worked on projects where I design test cases, document defects, and validate systems using tools such as JIRA and Postman. My background in quality control and process improvement allows me to bring an analytical and results-oriented approach.

## 🔗 Contacto
[![linkedin](https://img.shields.io/badge/linkedin-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sophia-mariel-castro)
📧 ing.sophiacastro@gmail.com
📱+52 6145227695

## 🛠 Skills
- Manual Testing
- Test Case Design
- Bug Reporting
- API Testing (Postman)
- SQL for Data Validation
- Excel advanced


## Projects
## 📊 Urban Grocers - API Testing Project

### Overview
This project focuses on API testing and backend validation using Postman.

### Objectives
- Validate API endpoints
- Ensure correct data handling and responses
- Identify backend defects

### Testing Activities
- Designed positive and negative test scenarios
- Created API checklists
- Tested endpoints using Postman
- Analyzed backend requirements using Apidoc

### Bug Reporting
- Documented critical defects in JIRA
- Contributed to improving system stability

### Results
- Improved system stability by 25% through defect identification

### Tools Used
- Postman
- JIRA
- Apidoc

## 📍Urban Routes - Manual Testing Project

### Overview
This project focuses on functional testing of a web/mobile application across multiple environments and versions.

### Objectives
- Validate application functionality
- Ensure consistency across environments
- Identify and document defects

### Testing Activities
- Designed and executed 20+ functional test cases
- Performed cross-environment and cross-version testing
- Analyzed application logic and UI behavior
- Created structured test documentation

### Bug Reporting
- Reported defects using JIRA
- Included detailed steps to reproduce, expected vs actual results, and severity

### Deliverables
- Test cases
- Bug reports
- Test documentation

### Tools Used
- JIRA
- Manual Testing Techniques


## 📈QA Data Validation Tool (Excel VBA)

### Overview
This project was applied in a manufacturing company, when simulates a data validation and control system designed to ensure data integrity and prevent input errors, similar to validation processes in software systems.

### QA Concepts Applied
- Input validation
- Duplicate prevention
- Data integrity checks
- Error handling
- User flow control

### Features
- Prevent duplicate entries
- Restrict invalid characters
- Structured data input
- Automated filtering
- Search functionality

### Testing Perspective
This tool reflects how validation rules and business logic are applied in QA to ensure reliable systems.



